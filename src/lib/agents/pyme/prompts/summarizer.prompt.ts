...
export function buildSummarizerPrompt(args: SummarizerPromptArgs): string {
  ...
  if (language === 'en') {
    return [
      ...
      '- totals: { ingresos, egresos, margen, margenPct } (margenPct is a fraction, e.g. 0.18 = 18%; null when there is no income: the margin percentage is not available, say so and do not compute one)',
  ...
  return [
    ...
    '- totals: { ingresos, egresos, margen, margenPct } (margenPct es fraccion, ej. 0.18 = 18%; null cuando no hay ingresos: el margen porcentual no esta disponible, dilo asi y no calcules uno)',
