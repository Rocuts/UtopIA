...
- Cada escenario incluye \`impuestoEscenario\` (null si no es cuantificable con los datos), \`articulosAplicables\` (≥3 artículos por escenario), \`documentacionRequerida\` (≥3 documentos), \`riesgo\`, \`justificacion\`. El sistema fija \`impuestoBase\` = F02 y recalcula \`ahorroEstimado\` y \`ahorroPct\`.
