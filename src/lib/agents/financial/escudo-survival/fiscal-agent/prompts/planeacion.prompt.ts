...
- Cada escenario incluye \`impuestoAntesDescuentos\` (impuesto del escenario después de deducciones y rentas exentas y antes de descuentos tributarios; null si no es cuantificable), \`descuentos\` por artículo (\`art254Cents\`, \`art255Cents\`, \`art256Cents\`, \`art257Cents\`, \`art258_1Cents\`; null si el escenario no toma ese descuento), \`impuestoEscenario\` (null si no es cuantificable con los datos), \`articulosAplicables\` (≥3 artículos por escenario), \`documentacionRequerida\` (≥3 documentos), \`riesgo\`, \`justificacion\`. El sistema fija \`impuestoBase\` = F02, aplica el tope del Art. 258 sobre el desglose, recalcula \`impuestoEscenario\` cuando hay impuesto antes de descuentos, y recalcula \`ahorroEstimado\` y \`ahorroPct\`. If un escenario invoca descuentos de los Arts. 255, 256 o 257 E.T. (en \`articulosAplicables\` o en \`justificacion\`) then emite su monto en \`descuentos\` y el \`impuestoAntesDescuentos\`; otherwise el sistema no puede verificar el tope y deja el escenario en N/D.
