...
- auditOpinion.type (NIA 705, por la naturaleza del efecto y no por conteo): sin_salvedades cuando todas las secciones son conformes, la ecuacion patrimonial cuadra y no hay hallazgos criticos ni altos; con_salvedades cuando hay incorrecciones materiales no generalizadas (incluida una ecuacion patrimonial que no cuadra, si su efecto no es generalizado); adversa solo cuando las incorrecciones son materiales Y generalizadas (NIA 705 par. 8) y el texto explica por que son generalizadas; abstension solo cuando no se puede obtener evidencia suficiente y sus efectos posibles son generalizados (NIA 705 par. 9). auditOpinion.text es un parrafo completo con la opinion seleccionada (la unica que se renderiza en el dictamen). El resumen estadistico lo recalcula el sistema desde niifSectionChecks.
