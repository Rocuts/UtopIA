...
- capitalizationProposal: applies=true cuando la utilidad neta del ejercicio aprobado es material (netIncomePrimary > $1.000.000); base = netIncomePrimary (v2.5 — utilidad del ejercicio, NO saldo acumulado PUC 36), amount = base × 0,40, body LITERAL incluyendo Ley 1258/2008 art. 5 + E.T. art. 36-3.
...
- MUST — FUENTE ÚNICA Y ESCALA DE CIFRAS DEL ACTA (corrección v2.5 #13). Todas las cifras monetarias del acta se COPIAN LITERALMENTE del Estado de Resultados y del Estado de Situación Financiera ya emitidos en TOTALES VINCULANTES (anchors Pass-1). PROHIBIDO recalcular, dividir entre 100/10⁸, multiplicar, redondear, re-derivar o tomar cifras de variables internas del sistema. Mapeo autoritativo (fuente → campo del acta):
    ...
    Base de capitalización (v2.5)        → Utilidad Neta del Ejercicio                    → ver capitalizationProposal abajo
    Monto a capitalizar (40%)            → Base × 0,40                                    → ver capitalizationProposal abajo
...
- MUST — CHECK previo a emitir el acta (Corrección v2.5 — ejecutar antes de devolver el JSON):
    ...
    base_capitalización_acta      == netIncomePrimary               (v2.5 — base es la utilidad del ejercicio)
    monto_capitalización_acta     == netIncomePrimary × 0,40        (v2.5 — 40% sobre la utilidad del ejercicio aprobada)
  Si cualquiera de los seis checks falla → NO emitir el acta; registrar la discrepancia en preparerNotes (NO en el body visible) hasta que cuadre.
...
If la utilidad neta del ejercicio aprobado en esta asamblea es material (netIncomePrimary > $1.000.000 COP) then capitalizationProposal.applies=true con:
  - base = netIncomePrimary (Utilidad Neta del Ejercicio del P&L — v2.5 corrección #13).
  - amount = base × 0,40 (40% de la utilidad del ejercicio que se está aprobando).
  - body LITERAL: "Capitalización del 40% de la utilidad del ejercicio aprobada. La Asamblea propone capitalizar el 40% de la utilidad neta del ejercicio que se aprueba en esta misma reunión, mediante reforma estatutaria conforme Ley 1258/2008 art. 5 (SAS)${isSAS ? ' — aplicable a esta entidad SAS, documento privado inscrito en Cámara de Comercio' : ' — aplicable a SAS; para sociedades reguladas por el C.Co. se requiere escritura pública'}. La base de la capitalización es la utilidad neta del ejercicio (mismo valor consignado en el Estado de Resultados aprobado en el orden del día previo) y el monto a capitalizar resulta de aplicar el 40% sobre dicha base. Para contexto patrimonial, las utilidades retenidas acumuladas (PUC 36) al cierre se reseñan en la sección 'Considerandos' del acta como información complementaria, pero NO constituyen la base de cálculo del 40%. Este movimiento queda exento del impuesto a los dividendos conforme E.T. art. 36-3, al constituir una reorganización patrimonial sin distribución efectiva (capitalización de utilidades, no su distribución)."
otherwise applies=false con base=$0 y amount=$0.
