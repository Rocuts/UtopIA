// Las pruebas de ruta sustituyen el Excel y el PDF por mocks, así que el render
// de las Partes IV/V sólo se ejercita aquí, contra ExcelJS real y el compositor
// real (portado de la PR #16 al modelo de procedencia de main).
...
describe('audit and meta-audit rendering in the workbook', () => {
  ...
  it('writes the opinion, the auditors and every consolidated finding', async () => {
    const wb = await load(await generateFinancialExcel({
      report: makeExportableReport(), auditReport: makeAudit(),
    }));
...
describe('audit and meta-audit rendering in the editorial PDF', () => {
  it('composes both pages only from a bound result', () => {
    const report = makeExportableReport();
    ...
    const doc = composeEditorialReport({
      ...
      auditReport: makeAudit(), qualityReport: makeQuality(), assuranceProvenance: 'server-persisted',
    });
