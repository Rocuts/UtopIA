...
describe('closingCredit', () => {
  ...
  it('integración I4: en inglés dice "Fiscal year"', () => {
    expect(closingCredit('2025', 'en')).toBe('Generated by 1+1 — Accounting and Tax Platform · Colombia · Fiscal year 2025');
    expect(closingCredit('', 'en')).not.toMatch(/\d{4}|Ejercicio/);
  });
});

describe('Contraportada en inglés (integración I4)', () => {
  it('un informe en inglés cierra con "Thank you for your trust", el año fiscal y el aviso en inglés', async () => {
    const doc = composeFixture();
    const pdf = await render({ ...doc, meta: { ...doc.meta, language: 'en' } });
    const closing = compact(pdf.pages[pdf.pages.length - 1]);
    expect(closing).toMatch(/Thank you/);
    expect(closing).toMatch(/for your trust/);
    expect(closing).toContain('Fiscal year 2025');
    expect(closing).toMatch(/does NOT replace the opinion/);
    expect(closing).not.toMatch(/Gracias|por confiar|Ejercicio 2025|NO sustituye/);
    // El título más largo no desborda: sin páginas en blanco.
    pdf.pages.forEach((text, i) => {
      expect({ page: i + 1, chars: withoutPageNumber(text).length >= 20 }).toEqual({ page: i + 1, chars: true });
    });
  }, 60_000);
