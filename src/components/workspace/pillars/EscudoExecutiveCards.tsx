...
/**
 * EscudoExecutiveCards — 4 tarjetas ejecutivas del Pilar Escudo.
 *
 * Vista del dueño/CFO: Autonomía · Cobertura Pasivos · Reserva Fiscal · Brecha Escudo.
 * Coexisten ARRIBA de los KPIs técnicos NIIF del pilar Escudo.
 * Datos 100% derivados del balance procesado por el Curator.
 *
 * Contrato visual:
 *   Autonomía         → azul    (sky)
 *   Cobertura Pasivos → naranja (amber)
 *   Reserva Fiscal    → morada  (violet)
 *   Brecha Escudo     → verde   (emerald)
 */
...
import type {
  ...
  ExecutiveCardColor,
  PillarStatus,
  EscudoExecutiveCards as EscudoExecutiveCardsData,
} from '@/lib/pillars/types';

interface Props {
  cards?: EscudoExecutiveCardsData;
  language: 'es' | 'en';
  density?: 'comfortable' | 'compact';
}

export function EscudoExecutiveCards({ cards, language, density = 'comfortable' }: Props) {
  if (!cards) return null;

  const order: Array<keyof Pick<EscudoExecutiveCardsData, 'autonomia' | 'cobertura_pasivos' | 'reserva_fiscal' | 'brecha_escudo'>> = [
    'autonomia',
    'cobertura_pasivos',
    'reserva_fiscal',
    'brecha_escudo',
...
/**
 * `key === 'autonomia'`: unit is 'ratio' in the type system but the raw value
 * represents days (see escudo-cards.ts line ~287). Render as integer days.
 */
...
/** Formato días. Más de 365 días = "∞ días" (liquidez prácticamente ilimitada). */
function formatDays(value: number, isEs: boolean): string {
...
export default EscudoExecutiveCards;
