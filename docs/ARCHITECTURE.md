...
- Without a reference, `/export` and `/html` recompute the same verdicts from the re-derived balance before the gate; a client `clean: true` never lifts them. Since the final round `/html` without reference builds the same `FinancialReport` from the three JSON (`htmlClientReport`) and runs it through `withServerRenderedClientReport` exactly like `/export` (verdicts of the three Partes including Parte I prose, identity of Partes II/III, `postProcessStrategyJson`, V1–V15 with scope `all`) and then `financialExportBlockers`: 422 before paying the Editor Jefe, which receives the verified JSON (derived Parte II figures). `/html` does not receive the trial-balance file, so the blockers that depend on the identity read from it (V5, V6) cannot be recomputed there: the UI forwards the report's `emittability` and post-render `validation` from `/consolidate` (`htmlReportVerdicts`), and a `no-emitible` or failed validation is folded in (hardening only; an `emittable` is ignored). By reference, `/export` and `/html` re-run them on the persisted version with the current rules.
...
- I5 (server remnants, contract `informe-niif-2026-09-24.3`):
  - Partes IV/V audit the server's text: `/api/financial-audit`, `/api/financial-quality` and `/api/fiscal-audit-opinion` pass their LLMs the consolidated report rebuilt from the Partes' JSON (`resolveAuditedReport` → `withServerRenderedClientReport`, with the re-derived preprocessed balance and the request ledger's adjustments trail), and derive the Parte IV integrity from it. A report without Partes I–III gets 422 `REPORT_PARTS_REQUIRED`. Without a reference, the Parte IV `auditReport` forwarded to Parte V is client text (LLM output with no JSON to re-render); `/api/fiscal-audit-opinion` always receives it that way.
  - Persisted Partes IV/V (`src/lib/reports/audit-result-version.ts`, `audit-result-store.ts`). With `reportRef`, `/api/financial-audit` and `/api/financial-quality` resolve the persisted version (`resolvePersistedReport`), run their agents over `withServerRenderedPersisted` of it with its persisted balance, and store the output **before responding** as a `reports` row (`kind = 'financial_audit_result'`) of the session's workspace. The row's jsonb holds the result in canonical JSON, the exact `reportRef` it was produced on, for Parte V the `auditRef` of the Parte IV it read (loaded from storage and required to be of the same version, else 409), a `complete` flag (false if an auditor failed or the agent returned nothing; a Parte V that read a partial Parte IV inherits false) and a SHA-256 of the whole envelope (`resultHash`). The response adds `auditRef`/`qualityRef = {resultId, resultHash}`, `auditComplete`/`qualityComplete` and `persistence`; if storage fails the result is returned `not_persisted` and without a reference, as `/consolidate` does. Report and audit content in the body are ignored. On the verified `/export` path the body's `auditReport`/`qualityReport` never reach the composer; `auditRef`/`qualityRef` are loaded with `loadBoundAuditResults` (same workspace → else 404; same id and hash, intact envelope, right Parte → else 409; produced on exactly the exported `{reportId, reportHash}`, complete, and Parte V paired with exactly the Parte IV sent, in both directions → else 409), and only then does `/export` pass `assuranceProvenance: 'server-persisted'`, the gate without which the composer omits both pages (reportes-export-11). Excel gets `Auditoria` and `Meta-auditoria` sheets from the same bound results. The stamp names each included result (id, date, fingerprint) and states that it vouches for provenance, not content (headers `X-Audit-Result-Id`/`-Hash`, `X-Quality-Result-Id`/`-Hash`); body content that was sent and dropped is declared (`X-Report-Audit-Content-Dropped`). The unverified path is unchanged.
...
- Final round of phase 2 (re-audit 2, contract `informe-niif-2026-09-24.4`):
  - Version format `v2` (`financial-report-version.ts`): `reportHash` is the fingerprint of the whole envelope (report, `sourceHash` —which binds the balance—, `rawDataHash`, contracts, `createdAt`, adjustments trail, language), so a row whose balance, fingerprints, contract or date were altered answers 409 `REPORT_VERSION_INTEGRITY` even with its `sourceHash` recomputed. `v1` rows are still read (their fingerprint covers only the report).
  - The version stores the Doctor de Datos trail (`adjustments`: applied + affected, `AdjustmentsTrail` in `adjustment-ledger.ts`) and the report language. The PDF appendix table "Ajustes Aplicados" lists them (id, account, period, previous → new balance, amount, rationale; without reference, from the request ledger), the HTML provenance banner lists them, and the stamp counts them and names both fingerprints (preprocessed balance *with* the adjustments, received file *before* them).
  - Parte I: a reconciliation deviation the code already overwrote in the JSON (`overwritten: true`) is trace, not a qualification (`hardenedReconciliation` counts only uncorrected ones; the JSON itself is still re-validated against the anchors).
  - Signatories: `renderGovernanceResult` prints the minutes' signatories and Revisor Fiscal from the intake (`company.signatories` or the legacy fields) and exposes the same identity in the JSON (`withIntakeSignatories`); without intake data, and always for the assembly's chair and secretary, "a completar al firmar". Never the model's names or IDs.
  - Draft reasons (`DraftReason`): besides the override, a non-emittable Editor Jefe HTML and a PDF with a composer watermark (BORRADOR for impracticable comparatives, INCOMPLETO, BLOQUEADO) get the *— BORRADOR* stamp with the reason and `X-Report-Draft: true` (in SSE the headers are already sent: the stamp and its `<meta>` carry it).
  - Language: without `language` in the body, by-reference outputs use the persisted version's language (the UI now sends it for the Excel too).
  - "Aplicar al reporte": the UI marks the report (`userEditedAt`) and warns next to the downloads that they do not include chat edits; `/export` without reference says so in the stamp and answers `X-Report-Edit-Dropped: true` (the server produces the Parts' text from their JSON, so the edit cannot reach the file).
  - The stamp states its scope (statement figures and the anchors the code verifies; AI narrative labelled "not audited" is not certified), and the Excel Summary tab labels the prose of each Parte I–III as AI narrative in the report language, like the PDF.
...
- Limits: the audit (Parte IV) and quality (Parte V) results are not part of the persisted version, so the PDF still omits client-sent ones; tenant isolation relies on the existing workspace resolution (session or anonymous cookie); `v1` versions (persisted before the final round) keep a report-only fingerprint and carry no structured adjustments trail (their consolidated report keeps the textual trail); the Editor Jefe input contract does not carry the adjustments ledger (the HTML discloses it in the provenance banner, outside the model's text).

### 5. Quality Meta-Auditor — best practices validation
`src/lib/agents/financial/quality/agent.ts`

Single agent evaluating the ENTIRE pipeline output against 12 quality dimensions. Frameworks: IASB Conceptual Framework, IFRS 18 readiness, ISO 25012 (data quality), ISO 42001 (AI governance), CTCP Colombia. Scores: overall grade A+→F, per-dimension scores, IFRS 18 readiness, data quality metrics, AI governance metrics. Entry point: `POST /api/financial-quality` (by reference: `reportRef` + optional `auditRef`, persisted and bound to the version; without it: report + auditReport + preprocessed, not persisted).
