...
- Without a reference, `/export` and `/html` recompute the same Parte II/III verdicts from the re-derived balance before the gate (`/export` for every path, `/html` via `serverActaVerdict`); a client `clean: true` never lifts them. By reference, `/export` re-runs them on the persisted version with the current rules.
