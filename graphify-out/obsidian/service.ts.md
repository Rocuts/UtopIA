---
source_file: "lib\accounting\double-entry\service.ts"
type: "code"
community: "getDb() / getOrCreateWorkspace()"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/getDb()_/_getOrCreateWorkspace()
---

# service.ts

## Connections
- [[createEntry()]] - `contains` [EXTRACTED]
- [[getEntryWithLines()]] - `contains` [EXTRACTED]
- [[isRetryableError()]] - `contains` [EXTRACTED]
- [[listEntries()]] - `contains` [EXTRACTED]
- [[lockOpenPeriod()]] - `contains` [EXTRACTED]
- [[nextEntryNumber()]] - `contains` [EXTRACTED]
- [[postEntry()]] - `contains` [EXTRACTED]
- [[retryOnSerialization()]] - `contains` [EXTRACTED]
- [[reverseEntry()]] - `contains` [EXTRACTED]
- [[validateAccounts()]] - `contains` [EXTRACTED]
- [[voidDraft()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/getDb()_/_getOrCreateWorkspace()