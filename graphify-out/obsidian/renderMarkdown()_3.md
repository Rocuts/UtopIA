---
source_file: "lib\agents\financial\audit\agents\tax-auditor.ts"
type: "code"
community: "tax-auditor.ts / renderMarkdown()"
location: "L179"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/tax-auditor.ts_/_renderMarkdown()
---

# renderMarkdown()

## Connections
- [[evaluacionIcon()]] - `calls` [EXTRACTED]
- [[fmtMoneyCop()]] - `calls` [EXTRACTED]
- [[fmtMoneyOrNa()]] - `calls` [EXTRACTED]
- [[fmtPctOrNa()]] - `calls` [EXTRACTED]
- [[priorityLabel()]] - `calls` [EXTRACTED]
- [[renderLegacyMarkdown()_1]] - `calls` [EXTRACTED]
- [[renderTaxDictamenMarkdown()]] - `calls` [EXTRACTED]
- [[riesgoIcon()]] - `calls` [EXTRACTED]
- [[tax-auditor.ts]] - `contains` [EXTRACTED]
- [[taxOpinionLabel()]] - `calls` [EXTRACTED]
- [[tmtIcon()]] - `calls` [EXTRACTED]
- [[toLegacyAuditorResult()_3]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/tax-auditor.ts_/_renderMarkdown()