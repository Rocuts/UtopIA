---
source_file: "lib\pillars\sync-validator.ts"
type: "code"
community: "sync-validator.ts / validateDashboardIntegrity()"
location: "L73"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/sync-validator.ts_/_validateDashboardIntegrity()
---

# validateDashboardIntegrity()

## Connections
- [[aggregateSeverity()_1]] - `calls` [EXTRACTED]
- [[checkAccountingEquation()]] - `calls` [EXTRACTED]
- [[checkEquityCoherence()]] - `calls` [EXTRACTED]
- [[checkEscudoCards()]] - `calls` [EXTRACTED]
- [[checkExecutiveCards()]] - `calls` [EXTRACTED]
- [[checkFuturoCards()]] - `calls` [EXTRACTED]
- [[checkValorKpis()]] - `calls` [EXTRACTED]
- [[checkVerdadCards()]] - `calls` [EXTRACTED]
- [[sync-validator.ts]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/sync-validator.ts_/_validateDashboardIntegrity()